...
class SyncNetInstance(torch.nn.Module):
    ...
    def evaluate(self, opt, videofile):
        ...
        command = ("ffmpeg -loglevel error -y -i '%s' -threads 1 -f image2 '%s'" % (videofile,os.path.join(opt.tmp_dir,opt.reference,'%06d.png'))) 
        ...
        flist = glob.glob(os.path.join(opt.tmp_dir,opt.reference,'*.png'))
        flist.sort()
